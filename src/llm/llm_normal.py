...
chat_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            f"modeがrequest：ユーザーの店舗選びの条件が入る。それをもとに別の外部のAIが優れた店をあなたに提示する。あなたは提示された全ての店の選定理由と懸念点を提示すること。もし店が提示されなかったらユーザーに左上の新しく店舗を探すボタンを押して条件を改めるように促す。提示は、店舗名,店舗の特徴,選定理由,の順で。表などを効果的に用いて、簡潔に端的に伝えること。最後に3つの店を比較して一番良い店をまとめること。提示後はユーザに店の詳細を聞いてもいいよなど選択肢を提示する\n\n"
            f"modeがchat：ユーザは提示した店舗に関する質問をする。条件の再入力は促さないこと。それ以降はユーザとの会話に場合によって検索ツールを使うこと\n\n"
            f"{tools}\n\nツールです。正しい名前と引数で呼んでね",
        ),
        ("system", "<<<MEMORY>>>\n{summary}\n<<<END>>>"),
